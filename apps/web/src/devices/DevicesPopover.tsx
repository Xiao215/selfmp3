 ...
 */
export function DevicesPopover({ onClose, up = true }: { onClose: () => void; up?: boolean }) {
  ...
  return (
    <>
      ...
      <div
        className={`popover devices-popover ${up ? 'popover-up' : ''}`}
        role="dialog"
        aria-label="Devices"
      >
        ...
        {others.length === 0 && (
          <p className="devices-empty">
            No other devices are open right now. Open self.mp3 on your phone and it will appear
            here.
          </p>
        )}
                  ...
                  {title ? (
                    <>
                      ...
                      {device.state.playing && device.state.position > 0 && (
                        <span className="device-at">
                          {' '}
                          · {formatDuration(device.state.position)}
                        </span>
                      )}
                    </>
                  ) : (
