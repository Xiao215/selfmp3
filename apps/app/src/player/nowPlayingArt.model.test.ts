...
const serverArt = (songId: number, rev: string): string =>
  `http://server/api/art/${songId}?v=${rev}`

describe('the Now Playing cover', () => {
  it('is the copy on this device whenever there is one', () => {
    expect(nowPlayingArtwork(song, { kept: 'file:///covers/7.jpg', serverArt })).toBe(
      'file:///covers/7.jpg',
    )
    expect(nowPlayingArtwork(song, { kept: 'file:///covers/7.jpg', serverArt: null })).toBe(
      'file:///covers/7.jpg',
    )
  })

  it('is the server’s address for a server library with nothing kept yet', () => {
    expect(nowPlayingArtwork(song, { kept: undefined, serverArt })).toBe(
