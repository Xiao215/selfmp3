 ...
 * that wants a caption says `data-tip="Up next"` instead, and this host —
 * mounted once in the shell — shows it: after a short rest on a mouse, at once
 * when moving along a row of controls, at once on keyboard focus, never on a
 * touch. A trailing `(key)` is drawn as a key cap.
 *
...
export function TooltipHost() {
  ...
  useEffect(() => {
    ...
    // Tab lands in focusin straight after, which puts the next caption up.
    ...
    // Only a scroll that carries the control away: synced lyrics scroll the
  ...
  useLayoutEffect(() => {
    ...
    layer.setAttribute('data-side', above ? 'above' : 'below')
  }, [shown])
  ...
  return createPortal(
    <div ref={layerRef} id={TOOLTIP_ID} role="tooltip" className="tooltip">
      {shortcut ? (
        <>
          {shortcut[1]}
          <kbd className="tooltip-key">{shortcut[2]}</kbd>
        </>
      ) : (
        shown.text
      )}
