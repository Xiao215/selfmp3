...
describe('the You page', () => {
  it('lists Stats, Untagged, Tags and Settings, each with what it has', () => {
    const rows = youRows({ plays: 328, untagged: 21, tags: 2 })
  ...
  it('lists Stats even with no plays to put beside it yet', () => {
    ...
    const rows = youRows({ plays: undefined, untagged: 4, tags: 3 })
    expect(rows.map(row => row.id)).toEqual(['stats', 'inbox', 'tags', 'settings'])
